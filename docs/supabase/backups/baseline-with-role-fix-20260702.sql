...
-- website_reader is a manually-created prod role this baseline GRANTs to.
-- Hosted preview branches do NOT apply supabase/roles.sql (local-only), so it
-- must exist before the grants below. No-op everywhere the role already exists;
-- never executes on prod (baseline is repair-marked applied there).
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'website_reader') THEN
    CREATE ROLE "website_reader" WITH LOGIN NOINHERIT;
  END IF;
END
$$;
